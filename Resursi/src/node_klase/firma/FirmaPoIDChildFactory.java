...
import static ERS.queries.ERSQuery.FirmaID;
 ...
 * @author dobri
 */
public class FirmaPoIDChildFactory extends ChildFactory<Firma> {

    private final int IDFirma;

    private class FirmaBeanNode extends BeanNode<Firma>
            implements IActionableKompanija {
    ...
    public FirmaPoIDChildFactory(int IDFirma) {
        this.IDFirma = IDFirma;
    }

    @Override
    protected boolean createKeys(List<Firma> list) {
        list.add(FirmaID(IDFirma));
        return true;
    }
